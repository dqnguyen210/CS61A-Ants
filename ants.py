...
class Bee(Insect):
    ...
    def action(self, colony, move_backw = False):
        ...
        # BEGIN EC
        if move_backw: destination = self.place.entrance
        # END EC
        if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while not place.bees and place.entrance != hive and count < self.max_range:
            place = place.entrance
            count += 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            same_place_bees = list(place.bees)
            for b in same_place_bees:
                Insect.reduce_armor(b,self.damage)
            place.bees = [b for b in same_place_bees if b.place == place]
        # END Problem 5
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        same_place_bees = list(self.place.bees)
        for b in same_place_bees:
            Insect.reduce_armor(b,self.damage)
        self.place.bees = [b for b in same_place_bees if b.place == self.place]
        # END Problem 7
...
class ScubaThrower(ThrowerAnt):
    """ ScubaThrower is a subclass of ThrowerAnt that can be placed on Water"""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.count_queen += 1
        if QueenAnt.count_queen == 1:
        ...
        if self.true_queen:
            ...
            while place:
                ...
                if ant:
                    ...
                    if ant.is_container and ant.contained_ant and ant.contained_ant not in self.buffed_ants:
...
def apply_effect(effect, bee, duration):
    ...
    def apply(*args):
        ...
        if duration > 0:
            duration -= 1
            return effect(old_action, bee)(*args)
        return old_action(*args)
    bee.action = apply
    # END Problem EC
